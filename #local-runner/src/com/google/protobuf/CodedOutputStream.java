...
public final class CodedOutputStream
{
  ...
  public static int computeStringSizeNoTag(String paramString)
  {
    ...
    {
      byte[] arrayOfByte = paramString.getBytes("UTF-8");
      return computeRawVarint32Size(arrayOfByte.length) + arrayOfByte.length;
    }
    ...
    throws IOException
  {
    ...
    else
    {
      ...
      else
      {
        ...
        if (paramInt1 != localInputStream.skip(paramInt1))
          throw new IllegalStateException("Skip failed? Should never happen.");
        while (paramInt2 > 0)
        {
          int j = Math.min(paramInt2, this.limit);
          int k = localInputStream.read(this.buffer, 0, j);
          if (k != j)
            throw new IllegalStateException("Read failed? Should never happen");
          this.output.write(this.buffer, 0, k);
          paramInt2 -= k;
        }
      }
    }
  ...
  public static int computeRawVarint64Size(long paramLong)
  {
    if ((paramLong & 0xFFFFFF80) == 0L)
      return 1;
    if ((paramLong & 0xFFFFC000) == 0L)
      return 2;
    if ((paramLong & 0xFFE00000) == 0L)
      return 3;
    if ((paramLong & 0xF0000000) == 0L)
      return 4;
    if ((paramLong & 0x0) == 0L)
      return 5;
    if ((paramLong & 0x0) == 0L)
